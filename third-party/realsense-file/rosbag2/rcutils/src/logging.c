...
//
// See
// https://developercommunity.visualstudio.com/content/problem/695656/wdk-and-sdk-are-not-compatible-with-experimentalpr.html
...
static const char * g_rcutils_logging_default_output_format = "[{severity}] [{time}] [{name}]: {message}";
#ifdef _WIN32
...
typedef const char * ( *token_handler )( const logging_input_t * logging_input,
...
static enum rcutils_get_env_retval
rcutils_get_env_var_zero_or_one( const char * name, const char * zero_semantic, const char * one_semantic )
{
    const char * env_var_value = NULL;
    const char * ret_str = rcutils_get_env( name, &env_var_value );
    if( NULL != ret_str )
    {
        RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING( "Error getting environment variable %s: %s", name, ret_str );
    ...
    RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING( "Warning: unexpected value [%s] specified for %s. "
...
static const char * expand_time( const logging_input_t * logging_input,
                                 rcutils_char_array_t * logging_output,
                                 rcutils_ret_t ( *time_func )( const rcutils_time_point_value_t *, char *, size_t ) )
{
    ...
    if( time_func( &logging_input->timestamp, numeric_storage, sizeof( numeric_storage ) ) != RCUTILS_RET_OK )
    {
...
static const char * expand_time_as_date( const logging_input_t * logging_input,
                                         rcutils_char_array_t * logging_output,
                                         size_t start_offset,
                                         size_t end_offset )
{
...
static const char * expand_time_as_seconds( const logging_input_t * logging_input,
                                            rcutils_char_array_t * logging_output,
                                            size_t start_offset,
                                            size_t end_offset )
{
...
static const char * expand_time_as_nanoseconds( const logging_input_t * logging_input,
                                                rcutils_char_array_t * logging_output,
                                                size_t start_offset,
                                                size_t end_offset )
{
...
static const char * expand_line_number( const logging_input_t * logging_input,
                                        rcutils_char_array_t * logging_output,
                                        size_t start_offset,
                                        size_t end_offset )
{
    ...
    if( logging_input->location )
    {
        ...
        int written = rcutils_snprintf( line_number_expansion,
                                        sizeof( line_number_expansion ),
                                        "%zu",
                                        logging_input->location->line_number );
        if( written < 0 )
        {
            RCUTILS_SAFE_FWRITE_TO_STDERR_WITH_FORMAT_STRING( "failed to format line number: '%zu'\n",
                                                              logging_input->location->line_number );
            return NULL;
        }
...
static const char * expand_severity( const logging_input_t * logging_input,
                                     rcutils_char_array_t * logging_output,
                                     size_t start_offset,
                                     size_t end_offset )
{
...
static const char * expand_name( const logging_input_t * logging_input,
                                 rcutils_char_array_t * logging_output,
                                 size_t start_offset,
                                 size_t end_offset )
{
...
static const char * expand_message( const logging_input_t * logging_input,
                                    rcutils_char_array_t * logging_output,
                                    size_t start_offset,
                                    size_t end_offset )
{
...
static const char * expand_function_name( const logging_input_t * logging_input,
                                          rcutils_char_array_t * logging_output,
                                          size_t start_offset,
                                          size_t end_offset )
{
    ...
    if( logging_input->location )
    {
        if( rcutils_char_array_strcat( logging_output, logging_input->location->function_name ) != RCUTILS_RET_OK )
        {
            RCUTILS_SAFE_FWRITE_TO_STDERR( rcutils_get_error_string().str );
            rcutils_reset_error();
            RCUTILS_SAFE_FWRITE_TO_STDERR( "\n" );
...
static const char * expand_file_name( const logging_input_t * logging_input,
                                      rcutils_char_array_t * logging_output,
                                      size_t start_offset,
                                      size_t end_offset )
{
    ...
    if( logging_input->location )
    {
        if( rcutils_char_array_strcat( logging_output, logging_input->location->file_name ) != RCUTILS_RET_OK )
        {
            RCUTILS_SAFE_FWRITE_TO_STDERR( rcutils_get_error_string().str );
            rcutils_reset_error();
            RCUTILS_SAFE_FWRITE_TO_STDERR( "\n" );
...
static const char * copy_from_orig( const logging_input_t * logging_input,
                                    rcutils_char_array_t * logging_output,
                                    size_t start_offset,
                                    size_t end_offset )
{
    ...
    if( rcutils_char_array_strncat( logging_output,
...
static void create_format_string( const char * logging_output_format_string )
{
    ...
    {
        back_slash_index
            = rcutils_findn( logging_output_format_string + i, RCUTILS_LOGGING_BACKSLASH_CHAR, length - i );
        if( back_slash_index == SIZE_MAX )
        {
            memcpy( g_rcutils_logging_output_format_string + dest_buffer_index,
...
static void parse_and_create_handlers_list( void )
{
    ...
    while( i < size )
    {
        ...
        {  // there is stuff before a token start delimiter
            ...
            i += chars_to_copy;
            if( i >= size )
            {  // perhaps no start delimiter was found
                break;
            }
...
{
    RCUTILS_CHECK_ALLOCATOR_WITH_MSG( allocator, "invalid allocator", return RCUTILS_RET_INVALID_ARGUMENT );
...
rcutils_ret_t rcutils_logging_initialize_with_allocator( rcutils_allocator_t allocator )
{
    ...
    const char * ret_str_level = rcutils_get_env( "RCUTILS_DEFAULT_LOGGER_DEFAULT_LEVEL", &env_default_level );
    if( NULL != ret_str_level )
    {
        RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING(
            "Error getting environment variable RCUTILS_DEFAULT_LOGGER_DEFAULT_LEVEL: %s",
            ret_str_level );
    ...
    else
    {
        ...
        int env_severity;
        rcutils_ret_t severity_ret = rcutils_logging_severity_level_from_string( env_default_level,
    ...
    {
        RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING(
            "Error getting environment variable RCUTILS_CONSOLE_STDOUT_LINE_BUFFERED: %s",
            ret_str );
    ...
    {
        RCUTILS_SAFE_FWRITE_TO_STDERR( "RCUTILS_CONSOLE_STDOUT_LINE_BUFFERED is now ignored. "
    ...
    retval = rcutils_get_env_var_zero_or_one( "RCUTILS_LOGGING_BUFFERED_STREAM", "not buffered", "buffered" );
    ...
    if( RCUTILS_GET_ENV_ZERO == retval || RCUTILS_GET_ENV_ONE == retval )
    {
        ...
        if( setvbuf( g_output_stream, NULL, mode, buffer_size ) != 0 )
        {
            ...
            RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING( "Error setting stream buffering mode: %s", error_string );
            return RCUTILS_RET_ERROR;
        }
    }
    ...
    retval = rcutils_get_env_var_zero_or_one( "RCUTILS_COLORIZED_OUTPUT", "force color", "force no color" );
    switch( retval )
    {
    case RCUTILS_GET_ENV_ERROR:
        return RCUTILS_RET_INVALID_ARGUMENT;
    ...
    rcutils_ret_t hash_map_ret = rcutils_hash_map_init( &g_rcutils_logging_severities_map,
                                                        2,
                                                        sizeof( const char * ),
                                                        sizeof( int ),
...
rcutils_ret_t rcutils_logging_shutdown( void )
{
    ...
    if( g_rcutils_logging_severities_map_valid )
    {
        ...
        if( hash_map_ret != RCUTILS_RET_OK )
        {
            RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING( "Failed to finalize map for logger severities: %s",
                                                      rcutils_get_error_string().str );
                                                          ...
                                                          int * severity )
{
    ...
    for( size_t i = 0; i < sizeof( g_rcutils_log_severity_names ) / sizeof( g_rcutils_log_severity_names[0] ); ++i )
    {
...
static rcutils_ret_t add_key_to_hash_map( const char * name, int level, bool set_by_user )
{
    ...
    if( hash_map_ret != RCUTILS_RET_OK )
    {
        RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING( "Error setting severity level for logger named '%s': %s",
                                                  name,
                                                  rcutils_get_error_string().str );
        return RCUTILS_RET_ERROR;
    }
...
int rcutils_logging_get_logger_leveln( const char * name, size_t name_length )
{
    ...
    if( short_name == NULL )
    {
        RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING( "Failed to allocate memory when looking up logger level for '%s'",
                                                  name );
        return -1;
    }
...
int rcutils_logging_get_logger_effective_level( const char * name )
{
    ...
    rcutils_ret_t hash_map_ret = rcutils_hash_map_get_size( &g_rcutils_logging_severities_map, &hash_map_size );
    if( hash_map_ret != RCUTILS_RET_OK )
    {
        RCUTILS_SET_ERROR_MSG_WITH_FORMAT_STRING( "Error getting severity level for logger named '%s': %s",
                                                  name,
                                                  rcutils_get_error_string().str );
        return -1;
    }
    ...
    while( true )
    {
        ...
        size_t index_last_separator = rcutils_find_lastn( name, RCUTILS_LOGGING_SEPARATOR_CHAR, substring_length );
...
rcutils_ret_t rcutils_logging_set_logger_level( const char * name, int level )
{
    ...
    if( level < 0
        || level >= (int)( sizeof( g_rcutils_log_severity_names ) / sizeof( g_rcutils_log_severity_names[0] ) ) )
    {
    ...
    if( rcutils_hash_map_key_exists( &g_rcutils_logging_severities_map, &name ) )
    {
        ...
        rcutils_ret_t hash_map_ret
            = rcutils_hash_map_get_next_key_and_data( &g_rcutils_logging_severities_map, NULL, &key, &tmp_level );
        while( RCUTILS_RET_OK == hash_map_ret )
        {
            ...
            hash_map_ret = rcutils_hash_map_get_next_key_and_data( &g_rcutils_logging_severities_map,
                                                                   &previous_key,
                                                                   &key,
                                                                   &tmp_level );
            if( hash_map_ret != RCUTILS_RET_OK && hash_map_ret != RCUTILS_RET_HASH_MAP_NO_MORE_ENTRIES
            ...
            if( free_current_key )
            {
                rcutils_ret_t unset_ret = rcutils_hash_map_unset( &g_rcutils_logging_severities_map, &previous_key );
...
static void vrcutils_log_internal(
    const rcutils_log_location_t * location, int severity, const char * name, const char * format, va_list * args )
{
...
void rcutils_log_internal(
    const rcutils_log_location_t * location, int severity, const char * name, const char * format, ... )
{
    va_list args;
    va_start( args, format );
    vrcutils_log_internal( location, severity, name, format, &args );
                                              ...
                                              rcutils_char_array_t * logging_output )
{
    ...
    {
        if( g_handlers[i].handler( &logging_input,
                                   logging_output,
                                   g_handlers[i].start_offset,
                                   g_handlers[i].end_offset )
            == NULL )
        {
            return RCUTILS_RET_ERROR;
        }
...
#define GET_HANDLE_FROM_STREAM( status, handle )                                                                       \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            {                                                                                                          \
                DWORD error = GetLastError();                                                                          \
                RCUTILS_SAFE_FWRITE_TO_STDERR_WITH_FORMAT_STRING( "GetStdHandle failed with error code %lu.\n",        \
                                                                  error );                                             \
                status = RCUTILS_RET_ERROR;                                                                            \
            }                                                                                                          \
        }                                                                                                              \
...
void rcutils_logging_console_output_handler( const rcutils_log_location_t * location,
                                             int severity,
                                             const char * name,
                                             rcutils_time_point_value_t timestamp,
                                             const char * format,
                                             va_list * args )
{
    ...
    char msg_buf[1024] = "";
    rcutils_char_array_t msg_array = { .buffer = msg_buf,
                                       .owns_buffer = false,
                                       .buffer_length = 0u,
    ...
    rcutils_char_array_t output_array = { .buffer = output_buf,
                                          .owns_buffer = false,
                                          .buffer_length = 0u,
    ...
    if( RCUTILS_RET_OK == status )
    {
        ...
        if( RCUTILS_RET_OK != status )
        {
            RCUTILS_SAFE_FWRITE_TO_STDERR_WITH_FORMAT_STRING( "Error: rcutils_char_array_vsprintf failed with: %d\n",
                                                              status );
        }
    }
    ...
    if( RCUTILS_RET_OK == status )
    {
        status = rcutils_logging_format_message( location, severity, name, timestamp, msg_array.buffer, &output_array );
        if( RCUTILS_RET_OK != status )
        {
            RCUTILS_SAFE_FWRITE_TO_STDERR_WITH_FORMAT_STRING( "Error: rcutils_logging_format_message failed with: %d\n",
                                                              status );
        }
    }
