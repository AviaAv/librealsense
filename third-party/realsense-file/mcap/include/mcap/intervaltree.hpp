...
public:
    ...
    interval_vector find_overlapping( const Scalar & start, const Scalar & stop ) const
    {
        ...
        visit_overlapping( start, stop, [&]( const interval & cur ) { result.emplace_back( cur ); } );
        return result;
    }
    ...
    {
        interval_vector result;
        visit_contained( start, stop, [&]( const interval & cur ) { result.push_back( cur ); } );
        return result;
    }
    ...
    std::pair< Scalar, Scalar > extent() const
    {
        ...
        visit_all( [&]( const interval & cur ) { extent( cur ); } );
        return extent.x;
    }
